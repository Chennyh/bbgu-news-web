      ...
      <el-table ref="userTable"
        ...
        <el-table-column label="角色" width="200" align="center">
          <template slot-scope="scope">
            <el-tag style="margin-left: 5px;margin-right: 5px;" v-for="role in scope.row.roles" :key="role.id">{{role.description}}</el-tag>
          </template>
        </el-table-column>
    ...
    methods: {
      ...
      handleAllocDialogConfirm(){
        this.$confirm('是否要确认?', '提示', {
          ...
          allocRole(this.allocUserId, params).then(response => {
            this.$message({
              message: '分配成功！',
              type: 'success'
            });
